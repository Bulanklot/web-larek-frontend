...
Методы:
- `_handleEscape`- обработчик закрытия по клавише `Escape`
- `_toggleModal` - метод для переключения модального окна
...
- `constructor(container: HTMLElement, events: EventEmitter)` - в конструкторе принимает контейнер - элемент разметки и экземпляр класса - брокер событий.
