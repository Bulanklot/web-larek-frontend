...
Конструктор будет принимать на вход данные выбранного товара из модели и будет заполнять поля модального окна в разметке.
...
```
Класс Form
Класс для создания форм с различным контентом.
 Выбирает все формы и вешает на них обработчики, обрабатывает ввод данных пользователя и отображает сообщения об ошибках

Свойства класса:

- submit - элемент кнопки для дальнейших действия после заполнения формы
- errors - элемент для отображения ошибок при заполнении формы
...
Свойства Класса:
-paymentOnline - элемент кнопки оплаты онлайн
...
```
Класс ContactForm
Класс для Реализации работой с формой контактных данных, наследует класс Form
...
```
Класс Success
Класс для отоборажения сообщения об успешной покупке.

Свойства класса:

- totalPrice - элемент для отображения общей суммы покупки
- сlose - элемент кнопки закрытия 
- event - для события клика по кнопке после завершения заказа
```
